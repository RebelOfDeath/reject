...
function eLongDivide(
        N: number[], D: number[]): { div: number[], rem: number[] } {
    ...
    if (D.length === 1) { 
        ...
        if (d === 1) { return { div: N, rem: [0] }; }
        if (d === -1) { return { div: eNegativeOf(N), rem: [0] } ; }
    }

    const signN = sign(N[N.length-1]);
    if (signN === 0) { return { div: [0], rem: [0] }}
    let signD = sign(d);
